...
Description:
	...
	Class for initialisation and logging od sensor 1
"""
## IMPORT DEPENDENCIES ##
...
class sens1:
	filename = 'log1/log0.csv'							# Log file - CSV format
	fileCnt = 0
	...
	def startup(self):
		...
		self.filename = ("log1/log{}.csv".format(str(self.fileCnt)))
	...
	def data_handler(self,content,data):
		...
		if self.cnt % 100 == 0:
			...
			if self.cnt2 == 10:
				...
				self.filename = ("log1/log{}.csv".format(str(self.fileCnt)))	# increment filename
				self.cnt2 = 0
			self.cnt2 += 1
